...
MetricsForBFFine::MetricsForBFFine(
    const FineBFHierarchicalAllocatorProcess& _allocator)
  : allocator(_allocator.self()),
    event_queue_dispatches(
        "allocator/mesos/event_queue_dispatches",
        process::defer(
            allocator,
            &FineBFHierarchicalAllocatorProcess::_event_queue_dispatches)),
    event_queue_dispatches_(
        "allocator/event_queue_dispatches",
        process::defer(
            allocator,
    ...
    allocation_run_latency("allocator/mesos/allocation_run_latency", Hours(1))
{
  ...
  foreach (const string& resource, resources) {
    Gauge total(
        "allocator/mesos/resources/" + resource + "/total",
        defer(allocator,
              &FineBFHierarchicalAllocatorProcess::_resources_total,
              resource));
...
void MetricsForBFFine::setQuota(const string& role, const Quota& quota)
{
  CHECK(!quota_allocated.contains(role));
  ...
  foreach (const Resource& resource, quota.info.guarantee()) {
    ...
    Gauge offered_or_allocated(
        ...
        "/offered_or_allocated",
        defer(allocator,
              &FineBFHierarchicalAllocatorProcess::_quota_allocated,
              role,
              resource.name()));
...
void MetricsForBFFine::removeQuota(const string& role)
{
  CHECK(quota_allocated.contains(role));
...
void MetricsForBFFine::addRole(const string& role)
{
  CHECK(!offer_filters_active.contains(role));
  ...
  Gauge gauge(
      "allocator/mesos/offer_filters/roles/" + role + "/active",
      defer(allocator,
            &FineBFHierarchicalAllocatorProcess::_offer_filters_active,
            role));
...
void MetricsForBFFine::removeRole(const string& role)
{
  Option<Gauge> gauge = offer_filters_active.get(role);
