  ...
  foreach (const SlaveID& slaveId, slaveIds) {
    ...
    foreach (const string& role, roleSorter->sort(slaveId)) {
      ...
      foreach (const string& frameworkId_, frameworkSorter->sort(slaveId)) {
        ...
        Try<Resources> targetResources = Resources::fromSimpleString(s);

        // If the total available resources (current "resources") cannot
        // satisfy the resource requirement specified by D-vector of this
        // framework, then skip this slave.
        if (!targetResources.isSome() ||
            !resources.contains(targetResources.get())) {
          continue;
        }

        resources = targetResources.get();
