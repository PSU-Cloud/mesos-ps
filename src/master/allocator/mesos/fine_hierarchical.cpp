...
FineHierarchicalAllocatorProcess::Framework::Framework(
...
void FineHierarchicalAllocatorProcess::initialize(
    ...
    const Option<DomainInfo>& _domain)
{
  ...
  PID<FineHierarchicalAllocatorProcess> _self = self();
  ...
  loop(
      ...
      [_self](const Nothing&) {
        return dispatch(_self, &FineHierarchicalAllocatorProcess::allocate)
          .then([]() -> ControlFlow<Nothing> { return Continue(); });
      });
}


void FineHierarchicalAllocatorProcess::recover(
...
void FineHierarchicalAllocatorProcess::addFramework(
...
void FineHierarchicalAllocatorProcess::removeFramework(
    const FrameworkID& frameworkId)
{
  CHECK(initialized);
  ...
  // FineHierarchicalAllocatorProcess::reviveOffers and
  // FineHierarchicalAllocatorProcess::expire.
...
void FineHierarchicalAllocatorProcess::activateFramework(
    const FrameworkID& frameworkId)
{
  CHECK(initialized);
  CHECK(frameworks.contains(frameworkId));
...
void FineHierarchicalAllocatorProcess::deactivateFramework(
    const FrameworkID& frameworkId)
{
  CHECK(initialized);
  CHECK(frameworks.contains(frameworkId)) << frameworkId;
...
void FineHierarchicalAllocatorProcess::updateFramework(
    const FrameworkID& frameworkId,
    const FrameworkInfo& frameworkInfo,
    const set<string>& suppressedRoles)
{
...
void FineHierarchicalAllocatorProcess::addSlave(
...
void FineHierarchicalAllocatorProcess::removeSlave(
    const SlaveID& slaveId)
{
  CHECK(initialized);
  CHECK(slaves.contains(slaveId));
  ...
  // FineHierarchicalAllocatorProcess::expire gets invoked (or the framework
...
void FineHierarchicalAllocatorProcess::updateSlave(
    const SlaveID& slaveId,
...
void FineHierarchicalAllocatorProcess::addResourceProvider(
    const SlaveID& slaveId,
    const Resources& total,
    const hashmap<FrameworkID, Resources>& used)
{
...
void FineHierarchicalAllocatorProcess::removeFilters(const SlaveID& slaveId)
{
  CHECK(initialized);
...
void FineHierarchicalAllocatorProcess::activateSlave(
    const SlaveID& slaveId)
{
  CHECK(initialized);
  CHECK(slaves.contains(slaveId));
...
void FineHierarchicalAllocatorProcess::deactivateSlave(
    const SlaveID& slaveId)
{
  CHECK(initialized);
  CHECK(slaves.contains(slaveId));
...
void FineHierarchicalAllocatorProcess::updateWhitelist(
...
void FineHierarchicalAllocatorProcess::requestResources(
    const FrameworkID& frameworkId,
...
void FineHierarchicalAllocatorProcess::updateAllocation(
    const FrameworkID& frameworkId,
    const SlaveID& slaveId,
...
Future<Nothing> FineHierarchicalAllocatorProcess::updateAvailable(
    const SlaveID& slaveId,
...
void FineHierarchicalAllocatorProcess::updateUnavailability(
    const SlaveID& slaveId,
    const Option<Unavailability>& unavailability)
{
  CHECK(initialized);
...
void FineHierarchicalAllocatorProcess::updateInverseOffer(
    const SlaveID& slaveId,
    const FrameworkID& frameworkId,
...
FineHierarchicalAllocatorProcess::getInverseOfferStatuses()
{
  CHECK(initialized);
...
void FineHierarchicalAllocatorProcess::recoverResources(
    const FrameworkID& frameworkId,
    const SlaveID& slaveId,
    const Resources& resources,
    const Option<Filters>& filters)
...
void FineHierarchicalAllocatorProcess::suppressOffers(
    const FrameworkID& frameworkId,
...
void FineHierarchicalAllocatorProcess::reviveOffers(
    const FrameworkID& frameworkId,
    const set<string>& roles_)
{
  CHECK(initialized);
  ...
  // `FineHierarchicalAllocatorProcess::expire` gets invoked. If we delete the
  // `OfferFilter` here it's possible that the same `OfferFilter` (i.e., same
  // address) could get reused and `FineHierarchicalAllocatorProcess::expire`
...
void FineHierarchicalAllocatorProcess::setQuota(
    const string& role,
    const Quota& quota)
{
  CHECK(initialized);
...
void FineHierarchicalAllocatorProcess::removeQuota(
    const string& role)
{
  CHECK(initialized);
...
void FineHierarchicalAllocatorProcess::updateWeights(
...
void FineHierarchicalAllocatorProcess::pause()
{
  if (!paused) {
...
void FineHierarchicalAllocatorProcess::resume()
{
  if (paused) {
...
Future<Nothing> FineHierarchicalAllocatorProcess::allocate()
{
...
Nothing FineHierarchicalAllocatorProcess::_allocate()
{
...
void FineHierarchicalAllocatorProcess::__allocate()
{
        ...
        if (framework.dvEnabled) {
          ...
          if (framework.dv.cpus > 0.0) {
            if (framework.dcapEnabled) {
              s = s + "cpus:" + std::to_string(std::min(framework.dv.cpus,
                    std::max(0.0, framework.dcap.cpus - allocatedCpus))) + ";";
            } else {
          ...
          if (framework.dv.mem > 0) {
            if (framework.dcapEnabled) {
              s = s + "mem:" + std::to_string(std::min(framework.dv.mem,
          ...
          if (framework.dv.cpus > 0.0) {
            if (framework.dcapEnabled) {
              s = s + "cpus:" + std::to_string(std::min(currcpus,
                    std::max(0.0, framework.dcap.cpus - allocatedCpus))) + ";";
            } else {
          ...
          if (framework.dv.mem > 0) {
            if (framework.dcapEnabled) {
              s = s + "mem:" + std::to_string(std::min(currmem,
                      std::max(0, framework.dcap.mem - allocatedMem))) + ";";
            } else {
...
void FineHierarchicalAllocatorProcess::deallocate()
{
...
void FineHierarchicalAllocatorProcess::_expire(
    const FrameworkID& frameworkId,
    const string& role,
    const SlaveID& slaveId,
...
void FineHierarchicalAllocatorProcess::expire(
    const FrameworkID& frameworkId,
    const string& role,
    const SlaveID& slaveId,
    OfferFilter* offerFilter)
    ...
    InverseOfferFilter* inverseOfferFilter)
{
  ...
  // FineHierarchicalAllocatorProcess::reviveOffers) but not yet deleted (to
...
bool FineHierarchicalAllocatorProcess::isWhitelisted(
...
bool FineHierarchicalAllocatorProcess::isFiltered(
    const FrameworkID& frameworkId,
    const string& role,
    const SlaveID& slaveId,
...
bool FineHierarchicalAllocatorProcess::allocatable(
    const Resources& resources)
{
...
double FineHierarchicalAllocatorProcess::_resources_offered_or_allocated(
...
double FineHierarchicalAllocatorProcess::_resources_total(
    const string& resource)
{
...
double FineHierarchicalAllocatorProcess::_quota_allocated(
    const string& role,
    const string& resource)
{
...
double FineHierarchicalAllocatorProcess::_offer_filters_active(
    const string& role)
{
  double result = 0;
...
bool FineHierarchicalAllocatorProcess::isFrameworkTrackedUnderRole(
    const FrameworkID& frameworkId,
...
void FineHierarchicalAllocatorProcess::trackFrameworkUnderRole(
    const FrameworkID& frameworkId,
    const string& role)
{
  CHECK(initialized);
...
void FineHierarchicalAllocatorProcess::untrackFrameworkUnderRole(
    const FrameworkID& frameworkId,
    const string& role)
{
  CHECK(initialized);
...
void FineHierarchicalAllocatorProcess::trackReservations(
...
void FineHierarchicalAllocatorProcess::untrackReservations(
    const hashmap<std::string, Resources>& reservations)
{
  foreachpair (const string& role,
               const Resources& resources, reservations) {
...
bool FineHierarchicalAllocatorProcess::updateSlaveTotal(
    const SlaveID& slaveId,
    const Resources& total)
{
  CHECK(slaves.contains(slaveId));
...
bool FineHierarchicalAllocatorProcess::isRemoteSlave(const Slave& slave) const
{
...
void FineHierarchicalAllocatorProcess::trackAllocatedResources(
    const SlaveID& slaveId,
    const FrameworkID& frameworkId,
    const Resources& allocated)
{
...
void FineHierarchicalAllocatorProcess::untrackAllocatedResources(
    const SlaveID& slaveId,
    const FrameworkID& frameworkId,
    const Resources& allocated)
{
