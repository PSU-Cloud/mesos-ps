...
FineBFHierarchicalAllocatorProcess::Framework::Framework(
...
void FineBFHierarchicalAllocatorProcess::initialize(
    ...
    const Option<DomainInfo>& _domain)
{
  ...
  VLOG(1) << "Initialized fine hierarchical allocator process";

  // Start a loop to run allocation periodically.
  PID<FineBFHierarchicalAllocatorProcess> _self = self();
  ...
  loop(
      ...
      [_self](const Nothing&) {
        return dispatch(_self, &FineBFHierarchicalAllocatorProcess::allocate)
          .then([]() -> ControlFlow<Nothing> { return Continue(); });
      });
}


void FineBFHierarchicalAllocatorProcess::recover(
    ...
    const hashmap<string, Quota>& quotas)
{
  ...
  if (quotas.empty()) {
    VLOG(1) << "Skipping recovery of fine hierarchical allocator:"
            << " nothing to recover";
...
void FineBFHierarchicalAllocatorProcess::addFramework(
...
void FineBFHierarchicalAllocatorProcess::removeFramework(
    const FrameworkID& frameworkId)
{
  CHECK(initialized);
  ...
  // FineBFHierarchicalAllocatorProcess::reviveOffers and
  // FineBFHierarchicalAllocatorProcess::expire.
...
void FineBFHierarchicalAllocatorProcess::activateFramework(
    const FrameworkID& frameworkId)
{
  CHECK(initialized);
  CHECK(frameworks.contains(frameworkId));
...
void FineBFHierarchicalAllocatorProcess::deactivateFramework(
    const FrameworkID& frameworkId)
{
  CHECK(initialized);
  CHECK(frameworks.contains(frameworkId)) << frameworkId;
...
void FineBFHierarchicalAllocatorProcess::updateFramework(
    const FrameworkID& frameworkId,
    const FrameworkInfo& frameworkInfo,
    const set<string>& suppressedRoles)
{
...
void FineBFHierarchicalAllocatorProcess::addSlave(
...
void FineBFHierarchicalAllocatorProcess::removeSlave(
    const SlaveID& slaveId)
{
  CHECK(initialized);
  CHECK(slaves.contains(slaveId));
  ...
  // FineBFHierarchicalAllocatorProcess::expire gets invoked (or the framework
...
void FineBFHierarchicalAllocatorProcess::updateSlave(
    const SlaveID& slaveId,
...
void FineBFHierarchicalAllocatorProcess::addResourceProvider(
    const SlaveID& slaveId,
    const Resources& total,
    const hashmap<FrameworkID, Resources>& used)
{
...
void FineBFHierarchicalAllocatorProcess::removeFilters(const SlaveID& slaveId)
{
  CHECK(initialized);
...
void FineBFHierarchicalAllocatorProcess::activateSlave(
    const SlaveID& slaveId)
{
  CHECK(initialized);
  CHECK(slaves.contains(slaveId));
...
void FineBFHierarchicalAllocatorProcess::deactivateSlave(
    const SlaveID& slaveId)
{
  CHECK(initialized);
  CHECK(slaves.contains(slaveId));
...
void FineBFHierarchicalAllocatorProcess::updateWhitelist(
...
void FineBFHierarchicalAllocatorProcess::requestResources(
    const FrameworkID& frameworkId,
...
void FineBFHierarchicalAllocatorProcess::updateAllocation(
    const FrameworkID& frameworkId,
    const SlaveID& slaveId,
...
Future<Nothing> FineBFHierarchicalAllocatorProcess::updateAvailable(
    const SlaveID& slaveId,
...
void FineBFHierarchicalAllocatorProcess::updateUnavailability(
    const SlaveID& slaveId,
    const Option<Unavailability>& unavailability)
{
  CHECK(initialized);
...
void FineBFHierarchicalAllocatorProcess::updateInverseOffer(
    const SlaveID& slaveId,
    const FrameworkID& frameworkId,
...
FineBFHierarchicalAllocatorProcess::getInverseOfferStatuses()
{
  CHECK(initialized);
...
void FineBFHierarchicalAllocatorProcess::recoverResources(
    const FrameworkID& frameworkId,
    const SlaveID& slaveId,
    const Resources& resources,
    const Option<Filters>& filters)
...
void FineBFHierarchicalAllocatorProcess::suppressOffers(
    const FrameworkID& frameworkId,
...
void FineBFHierarchicalAllocatorProcess::reviveOffers(
    const FrameworkID& frameworkId,
    const set<string>& roles_)
{
  CHECK(initialized);
  ...
  // `FineBFHierarchicalAllocatorProcess::expire` gets invoked. If we delete
  // `OfferFilter` here it's possible that the same `OfferFilter` (i.e., same
  // address) could get reused and `FineBFHierarchicalAllocatorProcess::expire`
...
void FineBFHierarchicalAllocatorProcess::setQuota(
    const string& role,
    const Quota& quota)
{
  CHECK(initialized);
...
void FineBFHierarchicalAllocatorProcess::removeQuota(
    const string& role)
{
  CHECK(initialized);
...
void FineBFHierarchicalAllocatorProcess::updateWeights(
...
void FineBFHierarchicalAllocatorProcess::pause()
{
  if (!paused) {
...
void FineBFHierarchicalAllocatorProcess::resume()
{
  if (paused) {
...
Future<Nothing> FineBFHierarchicalAllocatorProcess::allocate()
{
...
Nothing FineBFHierarchicalAllocatorProcess::_allocate()
{
...
void FineBFHierarchicalAllocatorProcess::__allocate()
{
  ...
  while (!roleHeap.empty()) {
    ...
    // again, slaveIds[0] in sort() is a dummy argument in DRF.
    foreach (const string& frameworkId_, frameworkSorter->sort(slaveIds[0])) {
      FrameworkID frameworkId;
      frameworkId.set_value(frameworkId_);
      ...
      foreach (const SlaveID& slaveId, roleSorter->bestFitSlaves()) {
        ...
        // The total resources we *can* offer to this framework,
        // ideally it should be the superset of resources specified
        // in this framework's D-vector.
        ...
        // we have allocatable revocable resources.
        if (!allocatable(resources)) {
          break;
        }
...
void FineBFHierarchicalAllocatorProcess::deallocate()
{
...
void FineBFHierarchicalAllocatorProcess::_expire(
    const FrameworkID& frameworkId,
    const string& role,
    const SlaveID& slaveId,
...
void FineBFHierarchicalAllocatorProcess::expire(
    const FrameworkID& frameworkId,
    const string& role,
    const SlaveID& slaveId,
    OfferFilter* offerFilter)
    ...
    InverseOfferFilter* inverseOfferFilter)
{
  ...
  // FineBFHierarchicalAllocatorProcess::reviveOffers) but not yet deleted (to
...
bool FineBFHierarchicalAllocatorProcess::isWhitelisted(
...
bool FineBFHierarchicalAllocatorProcess::isFiltered(
    const FrameworkID& frameworkId,
    const string& role,
    const SlaveID& slaveId,
    const Resources& resources) const
{
  ...
  // since they are not specific to the fine hierarchical allocator but rather
  // are global allocation constraints.
...
bool FineBFHierarchicalAllocatorProcess::allocatable(
    const Resources& resources)
{
...
double FineBFHierarchicalAllocatorProcess::_resources_offered_or_allocated(
...
double FineBFHierarchicalAllocatorProcess::_resources_total(
    const string& resource)
{
...
double FineBFHierarchicalAllocatorProcess::_quota_allocated(
    const string& role,
    const string& resource)
{
...
double FineBFHierarchicalAllocatorProcess::_offer_filters_active(
    const string& role)
{
  double result = 0;
...
bool FineBFHierarchicalAllocatorProcess::isFrameworkTrackedUnderRole(
    const FrameworkID& frameworkId,
...
void FineBFHierarchicalAllocatorProcess::trackFrameworkUnderRole(
    const FrameworkID& frameworkId,
    const string& role)
{
  CHECK(initialized);
...
void FineBFHierarchicalAllocatorProcess::untrackFrameworkUnderRole(
    const FrameworkID& frameworkId,
    const string& role)
{
  CHECK(initialized);
...
void FineBFHierarchicalAllocatorProcess::trackReservations(
...
void FineBFHierarchicalAllocatorProcess::untrackReservations(
    const hashmap<std::string, Resources>& reservations)
{
  foreachpair (const string& role,
               const Resources& resources, reservations) {
...
bool FineBFHierarchicalAllocatorProcess::updateSlaveTotal(
    const SlaveID& slaveId,
    const Resources& total)
{
  CHECK(slaves.contains(slaveId));
...
bool FineBFHierarchicalAllocatorProcess::isRemoteSlave(const Slave& slave) const
{
...
void FineBFHierarchicalAllocatorProcess::trackAllocatedResources(
    const SlaveID& slaveId,
    const FrameworkID& frameworkId,
    const Resources& allocated)
{
...
void FineBFHierarchicalAllocatorProcess::untrackAllocatedResources(
    const SlaveID& slaveId,
    const FrameworkID& frameworkId,
    const Resources& allocated)
{
