...
#ifndef __MASTER_ALLOCATOR_SORTER_RPSDSF_SORTER_HPP__
#define __MASTER_ALLOCATOR_SORTER_RPSDSF_SORTER_HPP__
...
class RPSDSFSorter : public Sorter
{
public:
  RPSDSFSorter();

  explicit RPSDSFSorter(
      const process::UPID& allocator,
      const std::string& metricsPrefix);

  virtual ~RPSDSFSorter();
...
private:
  ...
  friend RPSDSFMetrics;
  Option<RPSDSFMetrics> metrics;
};
...
struct RPSDSFSorter::Node
{
  ...
  // nodes are kept sorted by RPSDSF share.
  ...
  // Compares two nodes according to RPSDSF share.
  static bool compareRPSDSF(const Node* left, const Node* right)
  {
...
#endif // __MASTER_ALLOCATOR_SORTER_RPSDSF_SORTER_HPP__
