...
void PSDSFSorter::add(const string& clientPath)
{
...
void PSDSFSorter::remove(const string& clientPath)
{
  ...
  while (current != root) {
    ...
    if (current->children.empty()) {
      ...
      // accommodate inserting `clientPath` (see `PSDSFSorter::add()`),
...
void PSDSFSorter::activate(const string& clientPath)
{
...
void PSDSFSorter::deactivate(const string& clientPath)
{
  Node* client = CHECK_NOTNULL(find(clientPath));
...
void PSDSFSorter::updateWeight(const string& path, double weight)
{
  weights[path] = weight;
...
const hashmap<SlaveID, Resources>& PSDSFSorter::allocation(
...
const Resources& PSDSFSorter::allocationScalarQuantities(
    const string& clientPath) const
{
  const Node* client = CHECK_NOTNULL(find(clientPath));
...
hashmap<string, Resources> PSDSFSorter::allocation(const SlaveID& slaveId) const
{
...
Resources PSDSFSorter::allocation(
    const string& clientPath,
...
const Resources& PSDSFSorter::totalScalarQuantities() const
{
  return total_.scalarQuantities;
}


void PSDSFSorter::add(const SlaveID& slaveId, const Resources& resources)
{
  if (!resources.empty()) {
...
void PSDSFSorter::remove(const SlaveID& slaveId, const Resources& resources)
{
  if (!resources.empty()) {
...
vector<string> PSDSFSorter::sort(const SlaveID& slaveId) {
  ...
  // The children of each node are already sorted in PSDSF order, with
...
std::priority_queue<std::pair<std::string, double>,
                              ...
                              Sorter::ComparePair>
PSDSFSorter::yieldHeap(const SlaveID& slaveId) {
  // TODO(yuquanshan): since we need to re-sort when slave is changed,
  // the global (cross-slave) variable "dirty" is useless in our case,
  // so change "dirty" to "true" in if (.). Alternatively, we can add
  // another variable to, say, root node, which indicates which slave
...
double PSDSFSorter::updateVirtualShare(
...
bool PSDSFSorter::contains(const string& clientPath) const
{
  return find(clientPath) != nullptr;
}


int PSDSFSorter::count() const
{
  return clients.size();
}
...
double PSDSFSorter::findWeight(const Node* node) const
{
...
PSDSFSorter::Node* PSDSFSorter::find(const string& clientPath) const
{
