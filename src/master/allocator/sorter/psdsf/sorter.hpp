...
#ifndef __MASTER_ALLOCATOR_SORTER_PSDSF_SORTER_HPP__
#define __MASTER_ALLOCATOR_SORTER_PSDSF_SORTER_HPP__
...
#include "master/allocator/sorter/psdsf/metrics.hpp"
...
class PSDSFSorter : public Sorter
{
public:
  PSDSFSorter();

  explicit PSDSFSorter(
  ...
  // Add client to the sorter
  virtual void add(const std::string& clientPath);

  // Remove client from the sorter
  ...
  // Update weight on a role
  virtual void updateWeight(const std::string& path, double weight);

  // Update allocation on a client after resources from a slave have been
  ...
  // Return how many clients currently in this sorter
  virtual int count() const;

  // Util function printing all the tracked resources on each slave
...
struct PSDSFSorter::Node
{
  ...
  // nodes are kept sorted by PSDSF share.
  ...
  // Compares two nodes according to PSDSF share.
  static bool comparePSDSF(const Node* left, const Node* right)
  {
...
#endif // __MASTER_ALLOCATOR_SORTER_PSDSF_SORTER_HPP__
