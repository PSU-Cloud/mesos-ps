...
#ifndef __MASTER_ALLOCATOR_SORTER_TSF_SORTER_HPP__
#define __MASTER_ALLOCATOR_SORTER_TSF_SORTER_HPP__
...
#include "master/allocator/sorter/tsf/metrics.hpp"
...
// TODO(yuquanshan): The current TSF implementation is very awkward with
// roles and in cases where clients don't specify their D-vectors, since
// each role may cover multiple frameworks with different D-vectors and
// TSF itself won't work properly without knowing the true D-vector of
// the clients. So, this TSF only works properly when: 1. the frameworks
// under a role have to use the identical D-vector (currently the D-vector
// of a role is updated to the latest added framework's D-vector); 2. all
// the framework must specify it's D-vector. Otherwise, in more general
// cases, the scheduler would be screwed up by this mess.
class TSFSorter : public Sorter
{
public:
  TSFSorter();
...
struct TSFSorter::Node
{
  ...
  // nodes are kept sorted by TSF share.
  ...
  // Compares two nodes according to TSF share.
  static bool compareTSF(const Node* left, const Node* right)
  {
...
#endif // __MASTER_ALLOCATOR_SORTER_TSF_SORTER_HPP__
